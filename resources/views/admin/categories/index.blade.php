	...
	<div class="dashboard-content">
		...
		<div class="card">
					...
					<div class="card-body">
									...
									@foreach($objects as $object)
										<tr>
											...
											<td class="text-center">
												<form>
													<a href="javascript:void(0)" data-toggle="modal" data-id="{{$object->id}}" data-route="categories/{{$object->id}}"
														data-target="#myModal" class="edit show-object-data btn btn-sm btn-success">Izmijeni</a>
												</form>
                                            </td>
											<td class="text-center">
												<form class="deleteForm" action="{{ route('categories/delete', $object->id) }}" method="POST">
													@method('DELETE')
													@csrf
						...
						<a class="btn btn-sm btn-warning" href="{{ route('admin/categories/deleted') }}">Izbrisani objekti</a href="{{ route('admin/categories/deleted') }}">
					</div>
				</div>
			</div>
        </div>
